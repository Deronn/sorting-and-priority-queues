...
public class Greedy {
	...
	public static int GreedyAlgorithm(int[] table) {
		...
		// creating a priority queue which contains disk objects
		MaxPQ<Disk> PQ = new MaxPQ<>(table.length + 1);

		// creating the first disk to be inserted in PQ
		Disk disk0 = new Disk(0);

		// This will be used to count number of disks used in the text file
		int NumberOfDisks = 1;
	...
	/**
	 * Prints total number of disks used
	 * Prints disks in descending order
	...
	/**
	 * Checks if text file is in correct form.
	 * 
	 * @param file_path exact txt path
	 * @return the table which contains all files, null if the txt file is in incorrect form
	 * @throws IOException
	 */
